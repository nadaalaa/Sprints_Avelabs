...
void SWPWM_init(uint32 frequesncy_kh, uint8 dutyCycle)
{
    ...
    DIO_init(SWPWM_OUTPUT_PIN_SELECTOR,
    SWPWM_OUTPUT_PORT_SELECTOR,
     DIO_PIN_DIRECTION_OUTPUT);

    DIO_write(SWPWM_OUTPUT_PIN_SELECTOR,SWPWM_OUTPUT_PORT_SELECTOR,STD_LOW);
...
void SWPWM_TimerInterrupt_CallBack(void)
{
    ...
    switch(SWPWM_CurrentEdge)
    {
        case SWPWM_RisingEdge:
            DIO_write(SWPWM_OUTPUT_PIN_SELECTOR,SWPWM_OUTPUT_PORT_SELECTOR,STD_HIGH);
